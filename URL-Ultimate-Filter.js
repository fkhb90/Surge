 ...
 * @version     40.74 (引擎修正與規則重構)
 * @description 修正規則初始化邏輯，正確處理通配符與參數家族，恢復黑白名單功能。並基於此修正，全面重構與擴充參數規則庫。
...
const CONFIG = {
  ...
  GLOBAL_TRACKING_PARAMS: [
    ...
    'fb_action_types', 'fb_comment_id', 'fbclid', 'gci', 'gclid', 'gclsrc', 'gps_adid', 'gbraid', 'guce_referrer',
    'guce_referrer_sig', 'guccounter', 'hsCtaTracking', 'hsenc', 'iclid', 'igshid', 'ir_adid', 'ir_campaignid',
    'ir_partnerid', 'irclickid', 'is_retargeting', 'ko_click_id', 'li_fat_id', 'li_medium', 'li_source', 'linkedin_share',
    'mc_cid', 'mc_eid', 'mibextid', 'mindbox-message-key', 'msclkid', 'oly_anon_id', 'oly_enc_id', 'oprtrack',
    'rb_clickid', 'rtkcid', 'srsltid', 'sscid', 'trk', 'tt_adgroup', 'tt_campaign', 'tt_creative', 'tt_c_id',
    'ttclid', 'twclid', 'usqp', 'vc_lpp', 'vero_conv', 'vero_id', 'wbraid', 'wt_mc', 'xtor', 'yclid', 'ymid',
    'ysclid', 'zanpid'
  ].sort(),

  /**
   * 🗑️ [V40.70 新增] 裝飾性參數黑名單
...
class OptimizedTrie {
  ...
  startsWith(text) {
    ...
    // 檢查最後一個節點是否為單詞結尾（處理輸入字串與前綴詞完全相同的情況）
...
function initializeCoreEngine() {
  ...
  const listsToNormalize = [
    ...
    'DROP_KEYWORDS', 'GLOBAL_TRACKING_PARAMS', 'COSMETIC_PARAMS', 'TRACKING_PREFIXES',
    'PARAMS_TO_KEEP_WHITELIST'
  ];
