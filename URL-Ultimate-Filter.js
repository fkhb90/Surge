...
// 🚨 **修正**: Surge 日誌分類優化
const DROP_RESPONSE = { response: {} };                    // 完全丟棄，Surge 顯示「阻止」
const BLOCK_RESPONSE = { response: { status: 204 } };      // 無內容回應，Surge 顯示「阻止」
const REJECT_RESPONSE = { response: { status: 403 } };     // 拒絕存取，Surge 顯示「已修改」
...
function processRequest(request) {
    ...
    try {
        ...
        if (isCriticalTrackingScript(pathAndQuery)) {
            ...
            // JavaScript 檔案使用 DROP_RESPONSE 確保顯示「阻止」
            if (pathAndQuery.endsWith('.js') || pathAndQuery.includes('.js?')) {
                return DROP_RESPONSE;
            }
        ...
        if (isDomainBlocked(hostname)) {
            ...
            return BLOCK_RESPONSE; // 使用 204 確保顯示「阻止」
        }
