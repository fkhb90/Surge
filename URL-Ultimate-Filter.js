...
const CONFIG = {
  ...
  BLOCK_DOMAINS: [
    ...
    // --- [V40.70 新增] CNAME Cloaking / First-Party Trackers ---
    '2o7.net', 'demdex.net', // Adobe Analytics
    // --- 客戶數據平台 (CDP) & 身分識別 ---
    ...
    // --- [V40.70 新增] Video Ad Networks & VAST/VMAP ---
    'cedato.com', 'innovid.com', 'springserve.com',
    // --- 主流廣告聯播網 & 平台 ---
    ...
    // --- 雲端與平台分析/廣告像素 ---
    'bat.bing.com', 'cdn.vercel-insights.com', 'cloudflareinsights.com', 'everesttech.net', 'hs-analytics.net',
  ...
  PATH_BLOCK_KEYWORDS: [
    ...
    'pixanalytics', 'playtomic', 'polyad', 'popin', 'popin2mdn', 'popads', '/popup/', 'programmatic', 'pushnotification', 'quantserve',
    ...
    'zjtoolbar', 'zzhyyj',
    // --- [V40.70 新增] Video Ad Related ---
    '/midroll/', '/postroll/', '/preroll/', '/vast', '/vclick/', 'vpaid',
    // --- Ad Tech ---
    ...
    'amp-sticky-ad', 'amp4ads', 'apstag', 'google_ad', 'pagead', 'prebid', 'pwt.js',
    // --- Tracking & Analytics ---
  ...
  GLOBAL_TRACKING_PARAMS: [
    ...
    'awc', 'bemobdata', 'btag', 'cjevent', 'cjdata', 'cstrackid', 'cuid', 'dclid', 'external_click_id', 'fb_action_ids',
    'fb_action_types', 'fb_comment_id', 'fb_ref', 'fb_source', 'fbclid', 'gci', 'gclid', 'gclsrc', 'gps_adid', 'gbraid',
    'guce_referrer', 'guce_referrer_sig', 'guccounter', 'hsCtaTracking', 'hsenc', 'iclid', 'igshid', 'ir_adid',
    'ir_campaignid', 'ir_partnerid', 'irclickid', 'is_retargeting', 'ko_click_id', 'li_fat_id', 'li_medium', 'li_source',
    'linkedin_share', 'mc_cid', 'mc_eid', 'mibextid', 'mindbox-message-key', 'msclkid', 'oly_anon_id', 'oly_enc_id',
    'oprtrack', 'rb_clickid', 'rtkcid', 'srsltid', 'sscid', 'trk', 'tt_adgroup', 'tt_campaign', 'tt_creative',
    'tt_c_id', 'ttclid', 'twclid', 'usqp', 'vc_lpp', 'vero_conv', 'vero_id', 'wbraid', 'wt_mc', 'xtor', 'yclid', 'ymid',
    'ysclid', 'zanpid', 'gad_source', 'spot_im_redirect_source'
  ].sort(),
...
class MultiLevelCacheManager {
  ...
  getDomainDecision(h) {
    return this.l1DomainCache.get(h);
  }
  setDomainDecision(h, d) {
    this.l1DomainCache.set(h, d);
  }
  getUrlDecision(k) {
    const decision = this.l2UrlDecisionCache.get(k);
    if (decision !== null) optimizedStats.increment('l2CacheHits');
    return decision;
  }
  setUrlDecision(k, d) {
    this.l2UrlDecisionCache.set(k, d);
  }
  getUrlObject(rawUrl) {
    return this.urlObjectCache.get(rawUrl);
  }
  setUrlObject(rawUrl, urlObj) {
    this.urlObjectCache.set(rawUrl, urlObj);
  }
}
...
class OptimizedPerformanceStats {
  constructor() {
    ...
    this.labels = ['totalRequests', 'blockedRequests', 'domainBlocked', 'pathBlocked', 'regexPathBlocked', 'criticalScriptBlocked', 'paramsCleaned', 'hardWhitelistHits', 'softWhitelistHits', 'errors', 'l1CacheHits', 'l2CacheHits', 'urlCacheHits'];
  }
  increment(type) {
    const idx = this.labels.indexOf(type);
    if (idx !== -1) this.counters[idx]++;
  }
  getStats() {
    const stats = {};
    this.labels.forEach((l, i) => {
      stats[l] = this.counters[i];
    });
    return stats;
  }
}
...
function compileRegexList(list) {
  return list.map(regex => {
    try {
      ...
      logError(e, {
        rule: regex ? regex.toString() : 'invalid',
        stage: 'compileRegex'
      });
      return null;
    }
...
function getWhitelistMatchDetails(hostname, exactSet, wildcardSet) {
  if (exactSet.has(hostname)) return {
    matched: true,
    rule: hostname,
    type: 'Exact'
  };
  let domain = hostname;
  while (true) {
    if (wildcardSet.has(domain)) return {
      matched: true,
      rule: domain,
      type: 'Wildcard'
    };
...
function processRequest(request) {
  try {
    ...
    try {
      ...
      logError(e, {
        stage: 'urlParsing',
        url: getSanitizedUrlForLogging(rawUrl)
      });
    ...
    logError(error, {
      ...
      url: getSanitizedUrlForLogging(request ? .url)
    });
    return null;
  }
}
