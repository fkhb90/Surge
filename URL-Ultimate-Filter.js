 ...
 * @description 基於 V40.76 之全面優化：修正軟白名單傳參、L2 決策 TTL、Regex 前置放寬、
 *              PATH allow 前綴 Trie／後綴分桶、AC 高信度分層、採樣化統計與可選 Label 級 Trie。
...
let CACHE_EPOCH = 0; // 用於規則更新時 bump 使快取失效
function bumpCacheEpoch() { CACHE_EPOCH++; }

/* 計時與統計採樣：非除錯模式僅以 SAMPLE_RATE 記錄 */
const CONFIG = {
  DEBUG_MODE: false,
  TIMING_SAMPLE_RATE: 0.1, // 10%

  // 是否啟用標籤（label）級 Trie（實驗）
  EXPERIMENTAL_LABEL_TRIE: false,

  // Aho–Corasick 掃描字元上限（避免極長路徑掃描成本）
