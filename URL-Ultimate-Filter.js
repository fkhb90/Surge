 ...
 * @version     37.1 (YouTube Playback Hotfix)
 * @description V37.1 修復了因封鎖 ads.youtube.com 導致部分 YouTube 影片無法播放的問題。
...
const CONFIG = {
    ...
    BLOCK_DOMAINS: new Set([
        ...
        'log.pinterest.com', 'analytics.snapchat.com', 'ads-api.twitter.com', 'cint.com',
    ]),
...
const domainDecisionCache = new LRUCache(100);
...
function processRequest(request) {
    try {
        ...
        if (typeof console !== 'undefined' && console.error) {
            console.error(`[URL-Filter-v37.1] 處理錯誤: ${error.message}`, error);
        }
        return null;
    }
}
    ...
    try {
        ...
        if (typeof $request === 'undefined') {
            if (typeof $done !== 'undefined') {
                $done({ version: '37.1', status: 'ready', message: 'URL Filter v37.1 - YouTube Playback Hotfix' });
            }
            return;
        }
        ...
        if (typeof console !== 'undefined' && console.error) {
            console.error(`[URL-Filter-v37.1] 致命錯誤: ${error.message}`, error);
        }
...
// ### 📅 更新日期: 2025-09-08
//
// ### ✨ V37.0 -> V37.1 變更 (YouTube 播放問題修復):
//
// - **【錯誤修復】** 移除了域名黑名單中的 `ads.youtube.com`。
//   - **問題**: 經回報，封鎖此域名會導致部分 YouTube 影片無法正常播放。
//   - **原因分析**: YouTube 可能將影片播放前的必要性檢查（如地區限制或內容可用性驗證）與廣告系統的 API 部署在同一個子域名下。封鎖此域名會中斷播放流程。
//   - **解決方案**: 移除此條過於寬泛的域名攔截規則。腳本內建的其他路徑與參數過濾規則，已足夠攔截 YouTube 頁面上的廣告內容，無需封鎖整個子域名。
//
// =================================================================================================
// ## 更新日誌 (V37.0)
// =================================================================================================
