		...
		switch (this.platform()) {
			...
			default:
				...
				if (request.timeout) {
					...
					if (this.isSurge()) { /* Surge 使用秒 */ } else request.timeout = request.timeout * 1000;
				}
				if (request.policy) {
			...
			case 'Quantumult X':
				...
				if (request.body instanceof ArrayBuffer) {
					...
					delete request.body; // 修正：避免 delete object.body 錯誤
...
function setENV(name, platforms, database) {
	...
	console.log(`✅ Set Environment Variables, Settings: ${typeof Settings}`, "");
	/***************** Caches *****************/
	/***************** Configs *****************/
...
!(async () => {
	...
	switch (Settings.Switch) {
		...
		default:
			...
			let Panel = {};
			const connectInfo =
				`${Configs.i18n[Language]?.IPv4 ?? "IPv4"}: ` +
				`${Trace4?.ip ?? Configs.i18n[Language]?.Fail ?? "获取失败"}\n` +
				`${Configs.i18n[Language]?.IPv6 ?? "IPv6"}: ` +
				`${Trace6?.ip ?? Configs.i18n[Language]?.Fail ?? "获取失败"}\n` +
				`${Configs.i18n[Language]?.COLO ?? "托管中心"}: ` +
				`${Trace4?.loc ?? Trace6?.loc} | ` +
				`${Trace4?.colo ?? Trace6?.colo ?? Configs.i18n[Language]?.Fail ?? "获取失败"}\n` +
				`${Configs.i18n[Language]?.WARP_Level ?? "隐私保护"}: ` +
				`${Trace4?.warp?.toUpperCase() ?? Trace6?.warp?.toUpperCase() ?? Configs.i18n[Language]?.Fail ?? "获取失败"}`;
			// 填充面板信息
			switch ($.platform()) {
				case "Shadowrocket":
					break;
			...
			// 获取账户信息（避免與 env Caches 名稱衝突，使用 accountCaches）
			const accountCaches = $Storage.getItem("@Cloudflare.1dot1dot1dot1.Caches", {});
			if (accountCaches?.url && accountCaches?.headers) {
				// 構造請求
				let req2 = { "url": accountCaches?.url, "headers": accountCaches?.headers };
				// 获取账户信息
				const Account = await Cloudflare.fetch(req2).then(result => formatAccount(result?.account ?? {}, Language));
				const accountInfo = `\n`
					+ `${Configs.i18n[Language]?.Account_Type ?? "账户类型"}: `
					+ `${Account?.data?.type ?? Configs.i18n[Language]?.Fail ?? "获取失败"}\n`
					+ `${Configs.i18n[Language]?.Data_Info ?? "流量信息"}: `
					+ `${Account?.data?.text ?? Configs.i18n[Language]?.Fail ?? "获取失败"}`;
				// 填充
				switch ($.platform()) {
					case "Shadowrocket":
						break;
