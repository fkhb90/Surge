...
def _resolve_paths(args):
    ...
    candidates = [
        Path.cwd(),
        Path.cwd() / "rulesets",
        Path(r"C:\28Rule_Regression_Test\SSOT_Compiler\rulesets"),
        Path(r"P:\我的電腦\Surge\rulesets\Rules"),
    ]
    ruleset_dir = None
    for c in candidates:
...
def build_parser():
    ...
    p.add_argument("--ruleset-dir", type=str, help="rulesets 目錄路徑 (覆蓋預設)")
    ...
    p.add_argument("--repo-dir", type=str, help="git 倉庫根目錄 (預設自動從 ruleset-dir 向上偵測)")
    return p
...
def git_push(repo_dir: Path, ruleset_dir: Path, processed_tasks: list[tuple[str, str, str]]) -> bool:
    ...
    try:
        rel_set  = (ruleset_dir / "reject-set.list").relative_to(repo_dir)
        rel_drop = (ruleset_dir / "reject-drop.list").relative_to(repo_dir)
    except ValueError:
        rel_set  = (ruleset_dir / "reject-set.list").resolve()
        rel_drop = (ruleset_dir / "reject-drop.list").resolve()
    ...
    # 若存在未解決衝突，git commit 會直接失敗 (exit 128)
    unmerged = subprocess.run(
        gc + ["diff", "--name-only", "--diff-filter=U"],
        capture_output=True, text=True, encoding="utf-8"
    )
    conflict_files = [x for x in unmerged.stdout.splitlines() if x.strip()]
    if conflict_files:
        print(f"  {_red('ERR')} 偵測到未解決的 merge 衝突，無法自動 commit。")
        print("  請先手動解決以下檔案衝突後再執行 --push：")
        for f in conflict_files[:10]:
            print(f"    - {f}")
        if len(conflict_files) > 10:
            print(f"    ... 其餘 {len(conflict_files) - 10} 個檔案")
        return False

    if not run(gc + ["add", str(rel_set), str(rel_drop)], "git add"):
        return False

    status = subprocess.run(gc + ["diff", "--cached", "--stat"], capture_output=True, text=True, encoding="utf-8")
    ...
    if not run(gc + ["commit", "-m", msg], "git commit"): return False
    if not run(gc + ["push"], "git push"): return False
...
def main():
    ...
    # 若指定 --push，在任何檔案寫入前先同步遠端，確保工作區乾淨再 pull
    if args.push:
        ...
        if _pre_pull_repo_dir and _pre_pull_repo_dir.is_dir():
            gc_pre = ["git", "-C", str(_pre_pull_repo_dir)]
            print(f"  {_cyan('→')} 寫入前先同步遠端 (git pull --rebase --autostash)")
            r = subprocess.run(gc_pre + ["pull", "--rebase", "--autostash"],
                               capture_output=True, text=True, encoding="utf-8")
            if r.returncode != 0:
                print(f"  {_red('ERR')} git pull --rebase 失敗 (exit {r.returncode})")
                if r.stderr.strip(): print(f"  {_yellow('stderr:')} {r.stderr.strip()}")
                sys.exit(1)
            print(f"  {_green('✓')} 已同步至最新遠端狀態\n")
