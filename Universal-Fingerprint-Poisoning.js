// ==UserScript==
// @name         FP-Shield v2.60 (Test-Ready)
// @namespace    http://tampermonkey.net/
// @version      2.60
// @description  Universal Fingerprint Poisoning (JS-Injection Only). Derived from v2.60-Test-Ready.
// @match        *://*/*
...
(function() {
    ...
    // 將核心邏輯封裝為字串，透過 script tag 注入頁面 Main World
    // 這是為了確保能正確覆寫原生 prototype，而不受限於 Tampermonkey Sandbox
    const coreLogic = function() {
        ...
        const CONST = {
            // LocalStorage Keys
            KEY_SEED: "FP_SHIELD_SEED_V260_TM", 
            KEY_EXPIRY: "FP_SHIELD_EXP_V260_TM",
            ...
            // [Tampermonkey Limitation] Cannot remove CSP headers via JS. 
            // Setting to false as we cannot control headers here.
            REMOVE_CSP: false,     
            ...
            INJECT_MARKER: "__FP_SHIELD_V260_TM__"
        };

        // Check if already injected
        if (window[CONST.INJECT_MARKER]) return;
        Object.defineProperty(window, CONST.INJECT_MARKER, { value: true, writable: false });
        ...
        const isWhitelisted = WhitelistManager.check(window.location.hostname);

        const CONFIG = {
            ver: '2.60-TM-Test-Ready',
        ...
        const CanvasPool = (function() {
            ...
            return {
                ...
                release: function(p) { if(p && p.u) { p.u=false; p.t=Date.now(); } }
            };
        })();
            ...
            hardware: function(win) {
                try {
                    ...
                    if (!CONFIG.isIOS && !Persona.ch.mobile) {
                        ...
                        try { Object.defineProperty(N, 'mimeTypes', { get: () => ({ length: 0, item:()=>null, namedItem:()=>null }), configurable: true }); } catch(e) {}
                    }
                } catch(e) {}
            },
                ...
                try {
                    ...
                    const wrap = r => {
                        ...
                        if (win.DOMRectReadOnly) {
                            ...
                            Object.defineProperties(obj, desc); return obj;
                        }
                ...
                try {
                    ...
                    if (win.OffscreenCanvas) {
                        ProxyGuard.override(win.OffscreenCanvas.prototype, 'convertToBlob', (orig) => function() {
                            try {
                                if ((this.width*this.height) > CONFIG.canvasMaxNoiseArea) return orig.apply(this, arguments);
                                const ctx = this.getContext('2d');
                                if (ctx) { const d = ctx.getImageData(0,0,this.width,this.height); noise(d.data); ctx.putImageData(d,0,0); }
                            } catch(e){}
                            return orig.apply(this, arguments);
                        });
                    }
                    ...
                    [win.CanvasRenderingContext2D, win.OffscreenCanvasRenderingContext2D].forEach(ctx => {
                        ...
                        ProxyGuard.override(ctx.prototype, 'getImageData', (orig) => function(x,y,w,h) {
                            ...
                            if (w < CONFIG.canvasMinSize || (w*h) > CONFIG.canvasMaxNoiseArea) return r;
                            noise(r.data); return r;
                        });
                    });
                    ...
                    if (win.HTMLCanvasElement) {
                        ProxyGuard.override(win.HTMLCanvasElement.prototype, 'toDataURL', (orig) => function() {
                            ...
                            if (w < CONFIG.canvasMinSize || (w*h) > CONFIG.canvasMaxNoiseArea) return orig.apply(this, arguments);
                            let p=null;
                            try {
                            ...
                            finally { if(p) CanvasPool.release(p); }
                        });
                        ...
                        ProxyGuard.override(win.HTMLCanvasElement.prototype, 'toBlob', (orig) => function(cb, t, q) {
                            ...
                            const safeRel = () => { if(!released) { released=true; if(p) CanvasPool.release(p); } };
                            try {
                                p = CanvasPool.get(w, h);
                                p.ctx.clearRect(0,0,w,h); p.ctx.drawImage(this,0,0);
                                const d = p.ctx.getImageData(0,0,w,h); noise(d.data); p.ctx.putImageData(d,0,0);
        ...
        const inject = function(win) {
            ...
            Modules.webgl(win);
            // Modules.timezone(win); // Passthrough
            Modules.audio(win);
            Modules.rects(win);
            Modules.canvas(win);
        };
