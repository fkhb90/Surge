 ...
 * @version   2.63-Nuclear-HTML (Static Injection)
 * @description [核彈測試版] 放棄純 JS 渲染，直接將 UI 寫入 HTML 源碼，並移至左上角。
 * ----------------------------------------------------------------------------
 * 1. [Inj] Static HTML: 盾牌元素直接嵌入 response body，不依賴 JS 動態建立。
 * 2. [UI] Top-Left: 強制移至左上角，避開 iOS/Android 底部導覽列遮擋。
 * 3. [Debug] Red/Green: 紅色代表 JS 失敗(CSP阻擋)，綠色代表 JS 成功(防護啟動)。
 * ----------------------------------------------------------------------------
...
(function () {
  ...
  const CONST = {
    MAX_SIZE: 5000000,
    KEY_SEED: "FP_SHIELD_SEED_V263", 
    KEY_EXPIRY: "FP_SHIELD_EXP_V263",
    ...
    REMOVE_CSP: true,      // [Force] 開啟 CSP 移除，增加 JS 存活率
    ...
    INJECT_MARKER: "__FP_SHIELD_V263__"
  };
  ...
  // ============================================================================
  // 4. 靜態 HTML UI 注入 (Static HTML Injection) - The Nuclear Option
  // ============================================================================
  // 直接將 Div 寫死在 response 中，不依賴 JS 創建。
  // 樣式：左上角 (Top: 100px)，紅色背景 (預設)，最高層級 Z-Index
  ...
  <div id="fp-nuclear-badge" style="
      ...
      background-color: #D8000C !important; /* Start Red (Init) */
...
(function() {
  ...
  // ---------------- UI Updater ----------------
  // 負責尋找靜態注入的 Badge 並更新狀態 (Red -> Green/Gray)
  const UI = {
    update: function() {
       try {
           const b = document.getElementById('fp-nuclear-badge');
           if (!b) return; // Should not happen with static injection
           ...
           // Auto-hide after 5 seconds to prove visibility first
           setTimeout(() => {
  ...
  const init = function() {
    inject(window);
    UI.update(); // Update static UI if found
...
`;
  ...
  // 靜態注入 (Nuclear Option): 
  // 將 DIV 和 SCRIPT 拼接在一起。DIV 放在 Script 之前，確保它先被瀏覽器解析。
  const combinedInjection = staticBadgeHTML + injectionScript;

  // 嘗試插入到 body 結束標籤前，若無則 append 到最後
